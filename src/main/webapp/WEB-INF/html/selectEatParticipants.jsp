...
<body>
	...
	<a id="etnum" name="etnum" style="display: none">${etnum}</a>
    ...
    window.onload=function () {  
    	var etnum1 = document.getElementById("etnum").innerHTML;
        var url='${pageContext.request.contextPath}/eattype/selectUser'; //要请求的服务器url
        $.ajax({
            type: "POST",
            async:true,
            url:url, //请求路径
            data: {etnum:etnum1},  //请求参数
            dataType: "json",
            success: (list)=>{
                list.forEach((item)=>{
                	//alert(item.username);
                    ...
                    row.id=item.eatnum ; //把行id命名成标识符item.标识符属性名
	...
	<script>
		...
		var etnum = document.getElementById("etnum").innerHTML;
		...
		function save() {
			...
			var url = '${pageContext.request.contextPath}/eattype/insertEat'; //要请求的服务器url
			$
					.ajax({
						url : url,//要请求的服务器url
						data : {
		...
		function delForm(obj) {
	        ...
	        var url='${pageContext.request.contextPath}/eattype/deleteEat';  //要请求的服务器url
	        $.ajax({
	            url:url,//要请求的服务器url
	            data:{
	            	eatnum:flag
	            ...
	            success: function(data){
					...
					if (data.status==1){
						var etnum = document.getElementById("etnum").innerHTML;
						window.location.href="${pageContext.request.contextPath}/eattype/selectEatParticipants?etnum="+etnum;
						//location.href="http://www.baidu.com";
					}  
	            }
	        });
